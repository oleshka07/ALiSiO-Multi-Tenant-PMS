...
| 24 | `GET /restrictions` при нульовій наявності | не описано | `stop_sell` у ВІДПОВІДІ — ефективний прапорець: `true`, поки `availability` типу нуль, незалежно від збереженого. Збережений прапорець при цьому не губиться: `stop_sell: false` із ціною, надісланий при нулі, лягає — після `POST /availability` > 0 канал відкритий без повтору. І навпаки: саме зростання наявності збереженого `true` НЕ знімає — його знімає лише явний `false` (`scripts/channex-stop-sell-live.mjs`, 02.09.2026). Поруч: ніч без ціни — `rate: "0.00"`; `max_stay: 0` — «без межі»; `min_stay` на записі повертається як `min_stay_arrival` + `min_stay_through` |
