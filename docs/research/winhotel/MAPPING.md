...
| Референси | `GASTKREF.GK_LNR → GASTKONT`: `REF_NR` (номер броні OTA), `EXT_SOURCE` (на живому — теж число), `EXT_REFNR`, `REF_STORNO`, `PRECHECKIN_DONE`, `TEXT1..TEXT5` (вільний текст; назва каналу — тут, витягається з 10.09) |
...
| Наша ціль | `reservations` ← `GASTKONT` (1:1); `VERK_NR` → `parent_id`; **джерело — назва каналу за словом у текстових колонках `GASTKREF`** (`TEXT1..TEXT5`, потім `EXT_SOURCE`/`REF_NR`): на живому `EXT_SOURCE`, `REF_NR`, `EXT_REFNR` — ЧИСЛА (`18778622`, `2140901178` — номер броні каналу, рецензія Б2), а `MARKSEG` — ринковий сегмент, 0/null у 2 995 з 3 009 (рецензія Б); назва → `booking_sources`, номер → `reservations.hostex_reservation_code`; яка саме колонка несе назву на живому — каже рядок `explained` «колонка з назвою»; `BELEGUNG` з різними номерами на один `GASTKONT` → переселення (наш `reservation_sub_bookings` або історія) |
