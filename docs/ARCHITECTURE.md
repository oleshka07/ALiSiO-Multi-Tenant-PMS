...
- **Фаза 2/3 для Postgres** — асинхронний шар доступу. `better-sqlite3`
  синхронна, драйвер Postgres — ні; без цього `db/postgres/` нікуди не
  підключиться, а бекап окремого клієнта неможливий (SQLite — один файл).
  Це стеля масштабування, і вона не в швидкості: коли клієнтів троє і один
  просить відкотити помилковий імпорт, зробити це не зачепивши інших ніяк.
