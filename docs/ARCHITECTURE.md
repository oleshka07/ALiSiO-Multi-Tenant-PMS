...
- **Postgres.** Шов уже є: `core/db/async.ts` — інтерфейс `Sql` із чотирьох
  ...
  Перший модуль на шві — `dashboard`. Далі по одному, кожен окремим
  комітом: `reports` → `payments` → `tasks` → `notifications` → `channels`
  → `pricing` → `guests` → `properties` → `bookings` → `widget` →
  `finance` (найбільший, останній).
