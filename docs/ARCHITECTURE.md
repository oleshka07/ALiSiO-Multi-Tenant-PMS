...
- **Postgres.** Шов уже є: `core/db/async.ts` — інтерфейс `Sql` із чотирьох
  ...
  `tasks`, `notifications/daily-digest`, `pricing`. Далі по одному, кожен
  окремим комітом: `channels` (113 запитів) → `guests` (86) →
  `properties` (67) → `bookings` (160) → `widget` (180) → `finance`
  (336, останній).
  ...
  **Про автоматизацію цього переносу — застереження, куплене дорого.**
  ...
  І те, чого статична перевірка не побачить взагалі: `NextResponse.json`
  приймає `any`, тож передати туди проміс компілюється чисто і
  серіалізується в `{}`. У `pricing` таких місць було чотири — календар цін
  почав би відповідати екрану порожнім об'єктом, і `tsc` не сказав би
  нічого. Після кожного модуля треба руками пройти кожен виклик його
  репозиторію в обробниках і зробити живий GET/PUT.
