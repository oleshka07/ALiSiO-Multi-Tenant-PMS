...
| `property scope` (область обʼєкта) | два шари одного поняття. **В інтерфейсі** — який обʼєкт зараз на екрані: `?property=<id\|all>` в АДРЕСІ ВКЛАДКИ, кука `property_scope`, `usePropertyScope()`. **У запитах до API** те саме значення їде під іменем `?property_id=<id>` — так уже питають чотирнадцять екранів; порожнє значення означає «усі обʼєкти» СКАЗАНО, відсутнє — «нічого не сказано», і це різні стани. Обидва імені читає `requestedPropertyParam()`. **У даних** — тип `PropertyScope` (`{ kind: 'one'; id }` \| `{ kind: 'all' }`) з `@core/property-scope`, який читач scoped-таблиці приймає замість `property_id?: string`; `propertyScopeFilter()` перетворює його на `WHERE`, `requirePropertyScope()` — на двері з запиту | орендар — той на сесії й зʼєднанні; область його не заміняє. `propertyScopeSql()` з `properties/data/tenant-scope.ts` — попри імʼя, вісь ОРЕНДАРЯ («усі обʼєкти рахунку»), а не область; для нового коду двері — `propertyScopeFilter()`. Тип `PropertyScope` у `src/ui/PropertyScopeContext.tsx` — значення React-контексту, тобто третє значення того самого імені; перейменувати його на `PropertyScopeValue` — окремою правкою власника того файла |
