...
- **Статуси** — словники закриті і задокументовані тут:
  ...
  - `properties.checkin_payment_policy` (**чи пускають у номер до оплати**,
    0410): `prepaid` — як було до колонки \| `allow_pay_later` — гість
    заселяється, платить уранці на рецепції або при виїзді (КІ1). Слово
    обʼєкта, не пристрою: якби його носив кіоск, той самий готель відповідав
    би по-різному терміналу і власній рецепції на ту саму бронь
  - `properties.system_of_record` (**чия книга головна**, 0410): `alisio` —
    наша \| `external` — чужа система на час дзеркала. Не «слабше/сильніше»,
    а різне: від цього залежить, що кіоск шле при виїзді — фактуру з нашим
    номером чи підсумок перебування і рядок рецепції
  - `reservations.source = 'kiosk_walkin'` (**гість забронював сам і назвався
    на терміналі**, КІ8): попередня бронь, яку заводить кіоск, поки денна
    дельта чужої системи ще не прийшла. Це РІД походження, не канал: рядок у
    `booking_sources` тут не заводиться, інакше кожен готель мусив би завести
    його руками, перш ніж термінал запрацює. CHECK на цю колонку сьогодні НЕ
  ...
  - `kiosk_events.kind` (**що робили на терміналі**): вільний рядок —
    ...
    Свідомо без CHECK: словник подій росте швидше за міграції, а невідоме
    слово в ЖУРНАЛІ нічого не відчиняє — на відміну від статусу, за яким
    ухвалюють рішення. `kiosk_events.result` — навпаки, CHECK на трьох:
    `ok \| refused \| error`. Це вісь, за якою читають добу, і четверте
    слово зробило б підсумок неправдивим
  - Таблиці застосунку — префікс `kiosk_*`, кожна з `organization_id`; у
    таблиці ядра застосунок колонок не додає. Колонки на `properties` і
    `guest_registrations` (0410) — не виняток із цього, а навпаки: це
    політики й доказ, які належать ГОТЕЛЮ і живуть далі, коли термінал
    заберуть
