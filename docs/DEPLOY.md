...
Під сподом — `node scripts/issue-guest-app-key.mjs` у контейнері застосунку;
скрипт читає ключ НАЗАД окремим запитом, бо `UPDATE` без помилки не доводить,
що рядок змінився (політика могла відхилити його мовчки).
