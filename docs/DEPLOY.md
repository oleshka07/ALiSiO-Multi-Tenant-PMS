...
DATABASE_URL="postgres://alisio_app:…@127.0.0.1:54330/alisio" \
  node scripts/count-payer-folios.mjs
```
...
було таких рядків раніше. Очікується `жодного фоліо без броні й без фірми`.
Рядок, у якого немає ні броні, ні фірми, — це не фоліо платника: у «відкритих
фактурах фірми» його не буде взагалі (фільтр по `company_id`), а в списку
рахунків броні його немає теж. Скрипт тільки читає; вихід 1 означає «розібрати
до продакшну, а не після».
