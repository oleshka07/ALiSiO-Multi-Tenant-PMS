...
**Що лежить у sites-available — це `deploy/nginx/alisio.conf`, і так було не
завжди.** До 18.09.2026 скрипт клав повний шаблон, а через шістнадцять рядків
затирав його тимчасовим HTTP-конфігом (шаблон посилається на сертифікати,
яких ще немає), далі `certbot --nginx` дописував TLS у ТИМЧАСОВИЙ — і шаблон
не повертався ніколи, ні при першому запуску, ні при повторному. Ціна
вимірялась на живому готелі: блок
`location = /api/apps/winhotel-import/snapshots` з `client_max_body_size 200m`
не діяв у **жодному** середовищі й ніколи не діяв, тож агент отримував 413 на
знімок у 80 МБ. Тепер порядок такий: тимчасовий конфіг існує рівно доти, доки
сертифіката немає (при повторному запуску його не буде взагалі), а ОСТАННЄ, що
скрипт робить завжди, — кладе шаблон і перезавантажує nginx. Якщо шаблон не
проходить `nginx -t`, скрипт повертає попередній конфіг і виходить з помилкою,
**не перезавантаживши nginx**: зламаний конфіг тут кладе кожен інший сайт на
хості. Сертифікат один на обидва імені, і обидва серверні блоки називають
саме його теку — `/etc/letsencrypt/live/<domain>/`.

Наслідок для щоденної роботи: правку nginx роблять у `deploy/nginx/alisio.conf`
і застосовують повторним запуском `add-site.sh` (він ідемпотентний), а не
редагуванням файла на сервері — інакше наступний запуск її змете.
...
| 07:00 | «Kiosk heute»: лист рецепції про добу терміналів — у фазі `external` це РОБОЧИЙ список («виїхали, фактуру виставити в чужій системі»), у `alisio` — журнал. Обʼєкт без живого термінала пропускається мовчки | `deploy/run-cron.sh <env> /api/cron/kiosk-day` |
| кожні 5 хв | невідтверджені броні з гостьового застосунку, чий строк минув: номер повертається в продаж і в канал (КІ26; строк — 30 хв, `reservations.hold_expires_at`). Кожні пʼять хвилин, а не щогодини: кімната, яку тримає ніхто, це кімната, якої немає в продажу | `deploy/run-cron.sh <env> /api/cron/guest-app-holds` |
| кожні 5 хв | стрічки бронювань менеджера каналів | `deploy/run-cron.sh <env> /api/cron/channels-pull` |
| щохвилини | розсилка наявності й цін у менеджер каналів; застрягле доповідає, не червоніє (екран «Канал-менеджер») | `deploy/run-cron.sh <env> /api/cron/channels-publish` |
...
| щогодини :30 | диск ≥75% — алерт (26 серпня 100% поклали все); анти-спам: повторно лише при +5 п.п. | `deploy/check-cron.sh prod|beta` — звірка РОЗКЛАДУ: множина рядків із маркером
`# alisio-backup-<env>` у живому `crontab -l` мусить дорівнювати тій, що друкує
`./deploy/setup-backup-cron.sh <env> --list`. Зайвий рядок від старої версії
небезпечний так само, як відсутній. 19.09.2026 у продовому crontab стояло ТРИ
рядки з девʼяти — бракувало `channels-pull`, `channels-publish`,
`guest-app-holds` і `kiosk-day`, бо розклад ставить деплой, а прод не деплоївся
з 10.09: броні з каналів не приїжджали, ціни туди не їхали, холди не знімались —
і жодна з цих тиш не має вигляду помилки. Бігає наприкінці `deploy.sh` і в
`status.sh`; сам crontab не чіпає — розклад ставить деплой;
...
дамп із сотнею таблиць і 50 тис. бронювань відновлюється за секунди.

**Скільки таблиць має бути.** Живої бази — рівно на одну більше, ніж описує
`db/postgres/schema.sql`: зайва — `schema_migrations`, журнал накочених
міграцій, який створює сам `migrate.sh` і якого немає ні в SQLite, ні в
генераторі схеми. Тому «104 чи 105» — обидва правильні, просто про різні
бази: 104 у схемі нового клієнта, 105 на розгорнутій.

Число тут навмисно не записане: `restore-test.sh` рахує очікуване зі
schema.sql на місці й падає з поясненням, якщо відновлена база не збіглась.
Додалась таблиця — обидва числа зросли разом, і документація не встигла
збрехати.
...
## Перезавантаження хоста (runbook)

**Спершу головне: VPS спільний.** Ребут кладе не лише обидва середовища
PMS, а й СУСІДНІ проєкти на цій машині — на хвилини, поки все встає.
Вікно і саме рішення — за оператором; цей розділ лише про те, що
перевірити до і після, щоб ребут був нудним.

До ребута:

1. `./deploy/check-oom.sh prod && ./deploy/check-oom.sh beta` — головне
   тут навіть не OOM, а рядок `restart=` кожного контейнера: усе, що не
   `unless-stopped`/`always`, після ребута САМЕ НЕ ВСТАНЕ. Полагодити до
   ребута: app перестворює `./deploy/deploy.sh <env>`, postgres —
   `./deploy/apply-db-limits.sh <env>` (обидва беруть політику з
   docker-compose.yml).
2. `./deploy/check-backup-age.sh prod` — дамп і off-site свіжі. Сумнів —
   разовий `./deploy/backup.sh prod` перед ребутом.
3. Знати: деплой, що прилетить під час ребута, впаде на ssh-кроці —
   стара версія лишиться, це не аварія; після ребута його перезапускає
   кнопка Re-run у Actions.

Сам ребут — руками оператора (security-оновлення + reboot). Скрипта для
цього свідомо немає: момент і рішення людські, а разова команда на
власній машині — не те, від чого захищає §5 AGENTS.

Після ребута:

1. `./deploy/status.sh prod` і `./deploy/status.sh beta` — контейнери Up,
   health 401, диск.
2. `./deploy/check-oom.sh prod && ./deploy/check-oom.sh beta` — усе
   встало само, OOMKilled немає (рестарти = ребут, це очікувано).
3. `./deploy/smoke.sh prod && ./deploy/smoke.sh beta` — пʼять відповідей
   правильні по справжньому домену. Пʼята — POST на
   `/api/apps/winhotel-import/snapshots` тілом 30 МБ із завідомо невірним
   токеном: **401** означає «доїхало, відмовлено по праву», **413** — що
   nginx ріже тіло, тобто шаблон не застосований. 30 МБ, а не 11: серверний
   блок має власні 25m, тож менше тіло пройшло б і без блоку на 200m, і
   перевірка була б зеленою, не стверджуючи нічого. Нічого не зберігається —
   токен невірний, і відмова віддається до першого байта на диску.
4. Якщо щось не встало: `./deploy/logs.sh <env>`, і найпростіша кнопка
   підняти середовище цілком — `./deploy/deploy.sh <env>` (той самий sha,
   з дампом перед стартом).
...
**Останній крок — звірка готельних файлів**, і вона в образі, не на раннері:

```bash
docker exec -i alisio-prod-app node scripts/apply-hotel.mjs --all   # робить deploy.sh
docker exec -i alisio-beta-app node scripts/apply-hotel.mjs --all
```

`deploy.sh` викликає його сам; руками — щоб перевірити готель поза деплоєм.
Крок робить те, чого не робить жоден інший: піднімає НАШ код усередині
прод-образу. Двічі поспіль (06.09.2026, `8cdbddc` і `8cbefeb`) деплой бети
звітував «не пройшов» саме тут, при робочому застосунку й зеленому димі —
ланцюжок імпортів тягнув `next/server`, якого Next standalone не містить, а
повідомлення «готель не збігається зі своїм файлом» вказувало на дані
клієнта. Тому кожен скрипт, названий у цьому файлі як `node scripts/…`,
звіряється з `scripts/check-entry-imports.mjs`: названий тут і відсутній у
його списку — гейт червоний, і навпаки.
...
## Перед злиттям гілки в прод — репетиція на копії

```bash
./deploy/rehearse-merge.sh prod            # найсвіжіший дамп проду
./deploy/rehearse-merge.sh prod path.sql.gz
```

Запускається з робочої копії гілки, яку збираються злити (міграції й
`schema.sql` беруться поруч). Відновлює дамп в одноразовий Postgres, накочує
лише ті міграції, яких база ще не бачила (журнал `schema_migrations`, як
`migrate.sh`), звіряє число таблиць зі `schema.sql` + журнал, ганяє
`rls-check.sql` на мігрованій базі, накочує ті самі міграції ще раз
(неідемпотентна міграція зупинить другий деплой там, де перший пройшов) і,
якщо на хості є `psql` та `node`, — `check-schema-drift`. Друкує числа для
звіту: таблиці й політики до → після, скільки міграцій і за скільки мілісекунд,
організацій і бронювань у дампі. Живої бази не торкається. `FAILED` означає
рівно одне: у прод це не зливається, і скрипт назвав, чому.

До 05.09.2026 ця репетиція була пʼятнадцятьма рядками прози в ARCHITECTURE §8;
проза не запускається.

## Перед першим справжнім клієнтом

Чотири речі, і кожна тиха: жодна не падає на деплої, кожна проявляється як
...
```bash
# 4. Чи немає рахунків без броні, які НЕ є фоліо платника (0140)
./deploy/count-payer-folios.sh prod
```

`fin_folios.reservation_id` був нульовим у схемі задовго до 0140 — тобто база
дозволяла рахунок без броні ще тоді, коли код цього не вмів. 0140 навчила код
заводити фоліо **платника**, і питання перед переїздом клієнта одне: чи не
було таких рядків раніше.

Скрипт друкує три числа і **самі рядки** — `payer_kind`, `payer_name`,
`payer_debtor_no`, `label`, `created_at`. Присуду він не виносить навмисно:
рядок без броні й без фірми означає одне з двох, і розрізняє їх ДАТА. Створений
до 0140 — це «колонки тоді не існувало», і код тут ні до чого; створений після
— це справді дефект. Перша редакція цього інструмента писала «розібрати до
продакшну» на обидва випадки, і в беті це був неправильний діагноз при
правильному присуді.

> **Зʼєднанням застосунку (`alisio_app`) це НЕ запускається** — і саме тому
> скрипт бере його сам. Політика на `fin_folios` читає
> `current_setting('app.organization_id')` без другого аргументу, тож на
> невстановленому параметрі вона кидає `unrecognized configuration parameter`,
> і оператор дістає стек замість чисел. Підставити порожнього орендаря теж не
> можна: тоді питання звузилось би до одного орендаря і відповіддю був би
> втішний нуль. Скрипт ходить суперкористувачем із явним
> `SET row_security = off`, як `deploy/check-overlaps.sh`.

Заповнювати `company_id` таким рядкам **мовчки не можна** (Д60): зіставлення
за `payer_debtor_no` привʼязало б їх не до тієї фірми, бо зміст цього поля
змінився разом із 0140.

```bash
# 5. Наскільки розійшлись платіжні книги до переведення дверей (Д83)
./deploy/count-payment-gap.sh prod
```

Ревізія 16.09.2026 знайшла, що грошей приймали ДВОЄ дверей і писали в різні
книги: оплата з екрана фоліо лягала лише в рахунок гостя — ні рядка в касі, ні
слова на броні. Д83 звів двері в одні, але **вже записане не чіпає**:
переписувати заднім числом чужі гроші — рішення людини, не міграції.

Скрипт друкує три числа і рядки до кожного:

| Число | Що означає |
|---|---|
| готівка в рахунку гостя без парного рядка каси | гроші, яких не бачать ні P&L, ні залишок рахунку. Пара шукається за `folio_payment_id`, а не за сумою й датою: ті зійшлися б випадково |
| броней із двома номерами на ТІ САМІ гроші | ознака — участь «позначки оплати» (`folio_id IS NULL`): вона виписана на весь `total_price`, тож поруч із рахунком гостя це та сама сума двічі. Виправляється лише сторно. Два документи з ОДНОГО рахунку гостя сюди не входять — рахунок законно виставляє другий номер на пізніші нарахування, і це друге число поруч, без присуду |
| слово броні проти рахунку гостя | «борг нуль, а слово не оплачено» і навпаки — рівно те, що бачить рецепція й гість |

Нулі в усіх трьох означають, що розбіжності немає і робити нічого не треба.
Ненульове — це список для розбору, а не для автоматичної правки: готівка,
внесена до Д83, законно лишається в рахунку гостя без пари в касі.
...
Перед тим як обіцяти дату першому платному готелю — наскрізний прогін на
**порожній** базі. Він веде ДВА готелі від нуля до фактури, і два вони
навмисно: з одним готелем усе зламане виглядає цілим.
...
Шлях мусить бути **абсолютним** в обох процесах: `npm run start` піднімає
складання, і відносний `ALISIO_DATA_DIR` резолвиться у дві різні теки — прохід
тоді заводить готель в одній базі, а питає застосунок про іншу.

**Станом на 08.09.2026 прохід ЧЕРВОНИЙ**, і це не збій прогону: свіжозаведений
готель не може провести готівкову оплату (INC-028, чотири вади в ланцюжку —
план рахунків, касовий рахунок, згорнута в 500 відмова і зашита в CZK валюта
компанії). Зелений цей прохід — умова, за якої першого клієнта можна заводити
на прод.

## Курси валют після викочування 07.09.2026 — одноразово

До цього дня нічний прохід `sync-cnb-rates` писав **кожному** готелю з
обліковою валютою CZK чотири зашиті валюти (EUR, USD, GBP, PLN) — незалежно
від того, що готель оголосив у себе, і незалежно від `rate_source`. Курс,
який готель вписав удень, тієї ж ночі перекривався банківським.

Виправлення спиняє це наперед: прохід пропускає валюти, які готель фіксує сам.
Рядків, уже записаних у базу, воно **не чіпає** — і не може: у
`finance_exchange_rates` немає колонки походження, тож курс банку і курс
готелю після запису не відрізняються, а стирати «зайве» означало б стирати
курси, за якими могли бути виписані документи.

Тому дія одна, і робить її готель: **готель із фіксованим курсом мусить
внести його заново**. Новий рядок лягає сьогоднішньою датою і з цієї миті
виграє — `latestRate()` бере найсвіжіший `effective_from`.

Кого це стосується — видно скриптом (лише читає, нічого не пише):
...
Третій розділ його виводу («рядки, яких ніхто не просив») — це пари, що
лишились від старого проходу. Вони не показуються гостю, якщо валюти немає в
переліку показу, але їх бачить `latestRate()`, тобто і фактурування
(`invoicing/domain/fx.ts`). Ідеться про історію курсів, а не про гроші
готелю: рядки лишаються, поки хтось не назве конкретний, який справді
хибний.
...
## Гостьовий застосунок: адреса, куди веде QR на склі

```bash
./deploy/guest-app-key.sh beta                   # що є: обʼєкти і їхні адреси
./deploy/guest-app-key.sh beta demo-hotel        # видати ключ цьому обʼєкту
./deploy/guest-app-key.sh beta demo-hotel --rotate
```

Сторінка `/stay/<ключ>` — окремий застосунок, куди гість потрапляє з наліпки
на склі: розвилка «є бронювання / немає», пошук своєї броні за телефоном і
прізвищем, або підбір вільного номера з цінами і бронювання. Без ключа вона
не існує за жодною адресою, і це навмисно: ключ — це ПУБЛІЧНА адреса, за
якою будь-хто з інтернету бачить назву готелю і його вільні номери, тож
готель дістає її, коли попросив, а не при заведенні.

`--rotate` не «оновлює» ключ, а ВІДКЛЮЧАЄ старі наліпки: усі надруковані QR
перестають вести куди-небудь тієї ж секунди. Тому він питає підтвердження і
вимагає НАБРАТИ slug обʼєкта — Enter не досить.

**Головний шлях — не цей скрипт, а ЕКРАН**: адмінка → Налаштування →
Застосунки → «Гостьовий застосунок». Там орендар приходить із сесії, перелік
будинків і кнопка «Видати ключ» на картці. Скрипт лишається запасним — для
оператора біля сервера, коли до адмінки не дістатись.

**Вимикач важливіший за ключ.** Застосунок — платний ключ реєстру фіч
(`guest_app`, дефолт OFF), і поки він вимкнений, сторінка `/stay/<ключ>`
відповідає **404 навіть із виписаним ключем**. Скрипт каже про це окремим
рядком; на картці це видно самим вимикачем.

Під сподом скрипт читає й пише `psql` суперкористувача з
`SET row_security = off` — як `list-tenants.sh`. Перша редакція кликала
`node` у контейнері застосунку, і це НЕ ПРАЦЮВАЛО: застосунок ходить у базу
роллю `alisio_app`, а читання тенантної таблиці без орендаря віддає нуль
рядків мовчки (AGENTS §7). Оператор бачив порожній перелік і робив висновок,
що застосунків немає. Від `node` лишився рівно вигляд ключа (`--new`), щоб
алфавіт і довжина жили в одному місці.
