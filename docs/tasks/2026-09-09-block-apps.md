...
Наслідок для `dirs21` (уточнення власника 09.09): адаптер DIRS21 **буде** — як окремий
застосунок для готелів, що хочуть лишитись на DIRS21. Але в TourOnline AG **немає публічної
документації** (перевірено 07.09, `00-JOURNAL.md` §3.3), тож писати його можна лише після
Schnittstellenvertrag. Тому в **цьому** блоці `dirs21` — **картка «скоро» з кнопкою «хочу»**,
без коду; запит до TourOnline готує контролер, надсилає власник.
...
- Не писати адаптер DIRS21 **у цьому блоці** (специфікації ще немає — він прийде окремим
  застосунком після відповіді TourOnline) і не заводити для нього ключ реєстру.
