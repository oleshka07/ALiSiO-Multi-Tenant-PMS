...
| `10a416c3` | `55f73001`…`ef3ded69` (10 комітів: INC-038 вісь обʼєкта у фінансах, INC-045 п.1 спільна фікстура під орендарем — `check:pg` 38 → 64, П21, смуги номерів інцидентів) | `package.json` (`check` і `check:pg`: обидві сторони дописували гейти — обʼєднання, наш `apps.check` в обох, `check:pg` 65 сегментів) і три словники i18n (`catalogue`/`cs`/`de`: обидві сторони дописували ключі — обʼєднання, 3543 записи, наших 46 повернуто); зведено скриптом «чужа версія + наше, чого там немає», інших відмінностей від голови до злиття у цих файлах немає. `tsc` 0, `npm run check` зелений (1090 ok), `check:i18n` 3543/3543 |
| `2a362dcb` (циклом) | `3c9e8ed4` (лише `docs/research/winhotel/…` і задача хвилі Winhotel — коду нема) | немає; `tsc` 0, `npm run check` зелений; CI `34462617461` зелений (09:47–10:03, `check:pg` тепер ~14 хв через 65 сегментів). Запуски на `0122bde7` і `3dae4441` скасовані наступними пушами (concurrency), не збоєм |
| `6a6cb16b` | `bd5a54e8`…`8717ca47` (INC-045 п.2: подвійне бронювання забороняє БАЗА; Ґрайц — перший клієнт, Р13 частково скасовано) | `package.json` (`check`/`check:pg` — обидві сторони дописували гейти; обʼєднання, `apps.check` в обох, 219/66 сегментів) і `src/lib/db.ts` (обидві сторони дописали свій блок перед `[DB] migrations complete`: наш `migrateApps(database)` і їхній `ALTER reservations ADD is_pool_unit` — збережено обидва, наш перший, їхній останнім, як його коментар вимагає; вони незалежні, `migrateApps` не чіпає `reservations`). Цикл зупинено до злиття навмисно — на конфлікті він падає, а не зводить. `tsc` 0, `npm run check` зелений (1095 ok) |
| `e3e805fd` | `17ec6f01`…`e524a3e1` (9 комітів: INC-047 повтор лише на клієнтський ключ, INC-048/049 віджет одного будинку, П21/INC-100 доступ до рахунків, INC-101 перемикач готелів на свіжій базі) | `package.json` (`check`: обидві сторони дописували гейти — обʼєднання, `apps.check` в обох, 224/66 сегментів); `db.ts`, `schema.sql`, `platform/page.tsx`, ARCHITECTURE, DECISIONS злилися самі. Цикл упав на цьому конфлікті о 10:16 (він зводити не вміє) — зведено руками, цикл перезапущено. `tsc` 0, `npm run check` зелений (1117 ok) |
| `5c7e0504` (циклом) | `a656c945`…`4f8e0bcb` (6 комітів: INC-045 третє місце виклику й міграція 0132 → 0133, `check-text-files` бачить невідстежені файли) | немає; `tsc` 0, `npm run check` зелений; CI `34467488572` зелений (10:42–10:57). Попередні голови `fa315b18` (CI `34465938165` зелений) і злиття `e3e805fd` — див. рядок вище. Цикл після цього помер разом із перезапуском сесії (відʼєднаний процес не переживає контейнер) — перезапущено об 11:28 |
| `0ecdebc2` | `e5e9f8f2`…`2529ca03` (5 комітів: INC-101 `platform_memberships` як IDENTITY, звіти CI гілок, задачі хвилі Winhotel 11/11/9) | `package.json` (`check`/`check:pg` — обидві сторони дописували гейти; обʼєднання, `apps.check` в обох, 224/68 сегментів); решта злилась сама. Цикл упав на конфлікті об 11:37 — зведено руками, перезапущено. `tsc` 0, `npm run check` зелений (1120 ok) |
| `5ccf884f` (циклом) | `2529ca03` (inbox сесій 1–3, задачі хвилі Winhotel) | немає; `tsc` 0, `npm run check` зелений; CI `34472867051` зелений (11:44–12:01) |
| `f199b049` (циклом) | `24f79be3` (приймання перед `beta → main`: усе зелене, репетиція OK; правило AUTOLOOP «пуш іде за злиттям») | немає; `tsc` 0, `npm run check` зелений. Правило прочитане: воно про гілку робіт, на яку сесії спираються при старті. Для `claude/block-apps` лишаю порядок задачі 5 — злиття → `tsc` → `npm run check` → пуш: на цю гілку ніхто не спирається, а пуш червоної голови дав би червоний CI без користі |
| `4abed0fa` (циклом) | `59e53e08` (задача 12 сесії 2, AUTOLOOP: чия міграція; таблиця з віссю орендаря — з поясом у тій самій міграції) | немає; `tsc` 0, `npm run check` зелений. Правило про пояс перевірено на моєму 0140 (тепер 0400): `ENABLE`/`FORCE ROW LEVEL SECURITY` і політика у формі генератора для `app_connections` і `app_wishes` у тому самому файлі, проби в `rls-check.sql` — відповідає |
| `55bf5ca3` | `4acf2e7d`…`6fede7e2` (6 комітів: хвиля Winhotel §2 — дебітор, фоліо платника; INC-300 — згода на особі, злиття дублікатів гостя, міграція 0300) | три: `package.json` (`check`/`check:pg` — обʼєднання, `apps.check` в обох, 227/72 сегментів), `docs/NAMING.md` (обидві сторони дописали свій блок словника — збережено обидва) і `src/lib/db.ts` (обидві сторони дописали блок перед `[DB] migrations complete`: наш `migrateApps`, їхній INC-300 — збережено обидва, наш перший). Цикл упав на цьому о 13:37 — зведено руками. `tsc` 0, `npm run check` зелений (1141 ok); CI — див. `1764ec8d` нижче |

### Зіткнення номера міграції 0140 — перенумеровано в `0400–0402` (`1764ec8d`)

Злиття `55bf5ca3` принесло `0140-a-debtor-number-is-issued-by-the-hotel.sql` (сесія
фінансів, 10.09), а мій `0140-an-app-says-whether-it-is-connected.sql` живе на гілці з
09.09 — номер видала моя задача (§0: «міграції `0140–0149`»). Факти, за якими вирішено:

- **Накатці це не шкодить:** `deploy/migrate.sh` і `rehearse-merge.sh` ключують журнал
  `schema_migrations` повним імʼям файла, порядок — за імʼям; таблиці двох міграцій
  незалежні. Тобто це зіткнення смуг, а не поломка (такий самий давній дубль 0034
  живе з 26.08 і CI його терпить).
- **AUTOLOOP (10.09, «Смуги номерів: інциденти І МІГРАЦІЇ»)** віддає `0140…0199` сесії 1,
  а другій контрольній сесії і її сесіям — `0400…0499`; і каже прямо: «перейменовується
  той, чий номер ще НЕ в гілці робіт; злитий номер лишається». Мій номер не в гілці
  робіт, дебітор — у ній. Отже перейменовуюсь я, і в свою смугу.

Зроблено: три файли (`0140→0400`, `0141→0401`, `0142→0402`), заголовки міграцій,
згадки в `db.ts` (коментарі й тексти логів), `rls-check.sql`, ARCHITECTURE §2.3/§8,
NAMING, DECISIONS (З2, З17, З19) і цей звіт. Чужі згадки `0140` (дебітор, `companies`,
ARCHITECTURE про `companyPayer`) і текст задачі контролера не чіпались. Це поза
буквою задачі 5 («нічого іншого не робити»), але зіткнення створило саме злиття, а
правило AUTOLOOP чинне для всіх і називає, хто перейменовується; якщо контролер
вирішить інакше — це один `git mv` назад. Перевірки після перейменування:
`tsc` 0, `check-docs-current` 48 таблиць/39 гейтів, `check-decisions-registry` 216;
`check-schema-drift` локально не запустити (Postgres-стенд не пережив перезапуску
контейнера) — його робить CI крок «schema.sql and the migrations describe one
database». Повний `npm run check`: зелений (1141 ok).
| `9117b917` (циклом) | `1435dfa5`, `1e23da45` (inbox сесій 1 і 3; AUTOLOOP «кожна задача називає другий блок» — задачі 5 він не стосується, блок вичерпано) | немає; `tsc` 0, `npm run check` зелений; CI `34484988270` зелений (13:48–14:06) — перша голова з міграціями `0400–0402`, крок «schema.sql and the migrations describe one database» пройшов |

## Задача 6 — цикл зупинено

Зупинено за задачею 6, голова `9117b917` (CI `34484988270` зелений; цей рядок — коміт
поверх неї). `loop5.sh` завершився сам, знайшовши номер 6 о 14:03; годинне нагадування
знято; таймерів немає; дерево чисте. Гілку робіт більше не зливаю — `2cbff5bd` (AUTOLOOP,
вихід із циклу очікування) і все наступне зіллє контролер разом із фінальним злиттям.
Далі — лише цикл очікування з `MINE=6`, крок 10 хвилин.

## Задача 7 — блок закрито

Блок «Застосунки» закрито 10.09, голова `142d7071`. `beta → main` відбулося (~14:50);
гілку `claude/block-apps` зливає в гілку робіт контролер 1. Цикл очікування зупинено
(вийшов сам на номері 7), процесів і таймерів немає; сесію можна закрити.
