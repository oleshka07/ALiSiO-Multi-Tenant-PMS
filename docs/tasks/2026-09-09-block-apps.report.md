...
| `734a2984` + `(коміт нижче)` | `3d8dadd1`…`f6c29b48` (INC-046/047: ключ ідемпотентності віджета, `check-public-routes`, публічний контур числом) | `package.json`, рядок `check`: обидві сторони дописували свої гейти — зведено обʼєднанням (їхні `check-public-routes --strict` і `widget-reserve.idempotency.check` + наш `apps.check`). У `check:pg` наш гейт при зведенні випав і повернутий окремим комітом — інших відмінностей від голови до злиття у файлі немає |
