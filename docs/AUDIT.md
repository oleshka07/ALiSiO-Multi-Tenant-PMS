...
- **Мертві embed-скрипти на сайтах готелів (P0 для тих, хто їх поставив):** `public/widget/native-bundle.js` не перезібраний після фіксу `d1d52b4` — досі кличе видалений `/api/booking/checkout-session`; `service-embed.js:339` те саме + `:293` `GET /api/booking/promo` (перейменовано на `/activate`) → промокод завжди «Invalid»; ~~`collector.js:27` `POST /api/public/capture` (роуту нема) → ліди з форм губляться мовчки~~ — **виправлено**, §2.12.
