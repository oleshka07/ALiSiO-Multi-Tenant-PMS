...
- **Мертві embed-скрипти на сайтах готелів (P0 для тих, хто їх поставив):** ~~`service-embed.js:339` `/api/booking/checkout-session` + `:293` `GET /api/booking/promo`~~ і ~~`collector.js:27` `POST /api/public/capture`~~ — **виправлено**, §2.12, плюс гейт `check-embed-routes.mjs`. Лишається `public/widget/native-bundle.js`: зібраний React-бандл без джерела в репо, перезібрати нічим — рішення про долю «нативного» вбудовування за власником.
- ~~**Аналітика віджета не долітає:** `WidgetTab.tsx:92` роздає готелям `embed.v2.js`, а `postMessage`-протокол (purchase, UTM) розуміє лише `public/embed.v2.js` (корінь)~~ — **виправлено**, §2.12. При розборі знайшлося більше: **весь `/widget/` віддавав 307 на `/app/login`** — жоден embed-скрипт не вантажився в жодного готелю взагалі.
...
- ~~**«Завантаженість» — дві формули**~~ — **виправлено**, §2.12: одна формула в `@core/occupancy-rate` + гейт `occupancy-rate.check.ts`.
...
- ~~**`fees_taxes` недосяжна**~~ — **виправлено**, §2.12: збори заводяться секцією `fees` у файлі готелю (застосовуються деплоєм, ідемпотентно), арифметику винесено в `@core/…/fees.ts` із гейтом. Екран у налаштуваннях — окремою задачею.
...
- ~~`updateServiceOrder` IDOR~~ — **виправлено**, §2.12 (дірка виявилась ширшою: complete/cancel/reopen чужого замовлення, зі списанням чужого `booked_count`).
- ~~`group` edit дат без перевірки `checkOut>checkIn`~~ — **виправлено**, §2.12 (перевірка на СЕРВЕРІ; ночі рахує сервер зі своїх дат).
- ~~`changeStatus` на календарі — `if(res.ok)` без `else`~~ — **виправлено**, §2.12.
...
- ~~`test-email` — open-relay у межах тенанта~~ — **виправлено**, §2.12.
- ~~`.env.example` мертві ключі й відсутній `DB_DRIVER`~~ — **виправлено**, §2.12. Уточнення: живим виявився `NEXT_PUBLIC_APP_URL` (не `_DOMAIN`) — його лишено.
...
| E6 | (цей коміт) | Плитка «AI База знань» у налаштуваннях вела на `/app/settings/ai-knowledge` — сторінки немає; `ChatWidget` (499 рядків) шле `POST /api/ai/query` — маршруту немає; сам віджет не рендерить ніхто. Тобто «AI-ресепшн» — це плитка, яка веде в 404, і компонент, який ніхто не монтує, поверх бекенду, якого не існує. Прибрано і плитку, і компонент. Добудовувати це в межах розбору аудиту не можна: AI, який відповідає на «які сьогодні прибуття?», мусить ходити в базу — тобто це питання ізоляції орендарів і ключів моделі, а не кнопки. Приїде окремим модулем, коли для нього буде місце, ключі й межі; порожня плитка доти лише обіцяє те, чого немає. У PRODUCT.md AI-ресепшн ніколи й не був заявлений — там OpenAI згадано тільки для перекладу контенту й OCR, і це працює |
| E2/E7 | (цей коміт) | E7: «+ Додати рядок» і видалення рядка шлють `lineItems` без `subtotal`, а заголовок картки й індикатор ✅/⚠️Δ читають саме `subtotal` — тобто рядки рухалися, а число над ними лишалося старим, і індикатор розходився з арифметикою, яку будь-хто перерахує на папері. Полагоджено на сервері, а не в модалці: сума рядків — той самий факт для всіх, хто кличе цей PATCH, а клієнт, який про неї забув, — це і є те, як помилка сталася. Явний `subtotal` у тому ж запиті виграє (оператор навмисно перебиває суму); порожній список у суб-броні, яка мала рядки, дає нуль, а в тій, що ніколи їх не мала, не чіпає введену вручну суму. Попутно `DELETE` рядків переїхав усередину транзакції: він стояв зовні, тож збій на третьому вставленні лишав суб-бронь узагалі без рядків замість тих, що були. Доведено зворотним: вимкнув перерахунок — проба падає `999 !== 3500`. E2: у банері «Наступна дія» була дія «Запросити відгук» з `onClick: () => {}` — портьє натискав, нічого не відбувалося, і зрозуміти чому було ніяк. За нею немає нічого: ні шаблону листа, ні посилання на відгук у налаштуваннях об'єкта, ні маршруту відправки. Дію прибрано; запит відгуків приїде окремим модулем разом із цими трьома речами — пропонувати дію, якої немає, гірше, ніж не пропонувати |
| E3/E4 | (цей коміт) | E3: вкладка «Форми» була в панелі вкладок, компонента за нею не існувало — клік відкривав порожню панель. Таблиця `site_incoming_leads` при цьому є, лійка в аналітиці рахує її рядки, а міграція перелила в неї дані зі старої таблиці. Тобто це не «фічі бракує», а «хтось написав готелю, і цього ніхто не побачив». Написано `FormsTab` і `GET/PATCH /api/booking-sites/[id]/leads` через `withOwnedSite` — id сайту публікує віджет, а рядки містять ім'я, e-mail, телефон і текст стороннього. Лічильник вкладки — кількість непрочитаних. Попутно вирівняно форму таблиці: SQLite-бутстрап створював `source_url` і `raw_data`, яких немає в `db/postgres/schema.sql` і немає в живій SQLite-базі (вони вже стоять у списку мертвих колонок) — запит по них відповідав на одному двигуні й падав «no such column» на решті. E4: `SiteGiftCardsTab` — єдиний спосіб **випустити** подарунковий сертифікат — не був підключений до панелі, хоч погасити вже було можна; так само `OfferWorkflowTab` (кампанії кодів). Обидва написані повністю, з API й правами; додано вкладки «Сертифікати» й «Кампанії кодів». При підключенні знайдено дірку в `DELETE /api/gift-cards/workflow`: GET і POST були обмежені організацією, DELETE — ні, а він гасить усі невикористані коди правила. Доведено зворотним: прибрав `withOwnedSite` — проба падає «B read A's contact-form inbox: 200»; зелено на обох двигунах |
...
| §2.6 форми | (цей коміт) | `public/widget/collector.js` — снипет, який готель вставляє собі в `<head>` — шле кожну відправку форми в `POST /api/public/capture`. Такого маршруту **не існувало**: не «відповідав помилкою», а не було файлу. Власний `.catch(function () {})` збирача ковтав 404 навмисно, тож гість бачив «дякуємо», а звернення не доїжджало нікуди. Готель, який зробив усе за інструкцією, втрачав кожне звернення з дня, коли поставив снипет, і жодне місце про це не казало — ні браузер, ні лог, ні вкладка «Форми», яку ми написали вчора саме щоб їх показувати. Це найгірша форма помилки: кнопка, що падає, знаходиться за годину, а форма, яка каже «дякуємо» і викидає лист, — коли гість подзвонить спитати, чому ніхто не відповів. Маршрут написано: публічний за потребою (його кличуть з домену готелю браузером, який сесії не бачив), тож усе, що треба публічному запису, на місці — сайт мусить існувати (вигаданий id → 404, а не рядок у чужому інбоксі), у готелю мусить бути фіча `widget`, одна адреса має двадцять звернень на годину, пастка для ботів відповідає 200 і не пише нічого. Розкладання чужої форми в наші чотири колонки винесено в чистий `lead-form.ts` із гейтом `lead-form.check.ts`: `collector.js` серіалізує ті поля, які є на чужому сайті, а він писався не для нас — `jmeno` не рідше за `name`, `zprava` не рідше за `message`. Поле, під яке колонки немає, дописується в текст як «label: value», а не зникає: `raw_data` з таблиці прибрано як мертву колонку, і втратити частину написаного — та сама поразка, що й усе, тільки помітити важче. Попутно знайдено, що пастка для ботів у самому `collector.js` не могла спрацювати ніколи: серіалізатор викидав приховане поле, а `send()` жорстко ставив `_hp_trap = ''`, тобто «людина», незалежно від того, що там було. Виглядало як захист, було константою. Доведено зворотним, тричі: прибрав маршрут і перезібрав — проба падає рівно тим симптомом, що був у проді, `404` на `/api/public/capture`; вимкнув дописування несподіваних полів — гейт падає «a field we have no column for must survive as text»; вимкнув пастку — гейт падає. Проба виросла до 51 твердження і зелена на обох двигунах.
| §2.6 embed | (цей коміт) | Починав з рядка реєстру «аналітика віджета не долітає»: інтерфейс роздає `/widget/embed.v2.js`, а `postMessage`-протокол розумів лише кореневий `public/embed.v2.js`. Це правда, і це виявилось меншою половиною. **Перше.** Два файли на двох шляхах розійшлися, і жоден не був надмножиною: той, що роздають, умів ресайз, редирект, мову й прокидання параметрів; кореневий умів Meta/GA4/TikTok, `purchase` і UTM. Тобто готель, який заповнив Pixel ID на екрані налаштувань сайту й чекав конверсій, не отримував нічого: поля працювали, відстеження не існувало. Тепер один файл — об'єднання обох, — і `next.config.ts` переписує старий URL на нього, бо чужий сайт ми редагувати не можемо. **Друге, важливіше.** `/widget/` ніколи не був у списку публічних шляхів `src/proxy.ts`, а матчер виключає svg, png, css, woff… але **не `.js`**. Тож `embed.v2.js`, `service-embed.js`, `collector.js`, `native-embed.js` — усі віддавали **307 на `/app/login`**. Сайт готелю вантажив тег, ішов за редиректом і виконував HTML сторінки логіну як JavaScript. Ніякого віджета, ніякого збирача форм; у консолі — лише що скрипт не розібрався. `embed.css` при цьому працював (css у списку виключень), через що це виглядало як проблема стилів. Зламано з дня, коли з'явився захист маршрутів: `git log -S` каже, що `/widget/` не був публічним ніколи. **Третє.** `?lang=` — найвищий пріоритет мови у віджеті, а embed підставляв власний запасний `'uk'`, коли не міг вгадати. Тобто вчорашній фікс F1 не доїжджав до жодного готелю зі стандартним снипетом: німецький готель показував бронювання українською кожному, чий браузер не влучив у чотири мови. Тепер embed не вигадує мову: не знає — не шле параметр, і виграє мова готелю. Дорогою: пастка для ботів у `collector.js` не могла спрацювати (серіалізатор викидав приховане поле, `send()` жорстко ставив «людина»); `public/widget/test-services.html` — чернетка, зашита на `localhost:3002`, — прибрана, бо `/widget/` тепер публічний; `/manifest.json` теж відкрито (браузер тягне його на сторінці логіну, де сесії за визначенням немає). Доведено зворотним: повернув `/widget/` під захист і перезібрав — проба падає «/widget/embed.v2.js is not reachable without a session (307) — every hotel's embed is dead». Проба виросла до 52 тверджень (перевіряє 200 без кукі, що це не HTML, що обидва URL — байт у байт один файл, що `purchase` на місці й що мова не вигадується), плюс `widget-language.check.ts` тепер звіряє список мов у скрипті зі списком у коді. Операторський застосунок лишається закритим: `/app/settings` 307, `/api/bookings` 401.
| §2.6 послуги | (цей коміт) | `service-embed.js` — віджет сауни, купелі й сніданку, який стоїть і на гостьовій сторінці, і на сайтах готелів — був мертвий з обох кінців. Промокод ішов у `/api/booking/promo`, перейменований на `/activate`: фетч давав 404, `catch` перетворював це на «Invalid», і кожен код, який готель випустив — включно з проданими подарунковими пакетами, — його ж віджет відхиляв. Оформлення йшло в `/api/booking/checkout-session`, видалений разом із Teya: гість обирав час, вводив код, тиснув «оплатити» і отримував «Payment init failed» — у момент, коли вже вирішив платити. Ремонт виявився відніманням, а не додаванням: `doSubmitSlot`/`doSubmitBreakfast` уже писали справжнє бронювання через живий `/api/booking/services` — зі слотом, годинами, додатками й окремим рядком на кожен ранок сніданку — і робили це завжди. Мертвий шлюз сидів гаком між кнопкою і ними. Гак прибрано, разом із обробником повернення з оплати (нікуди не редиректить — отже, нізвідки повертатися) і полями сесії платежу, яких ніхто не читав. Підпис кнопки — «замовити», бо це те, що вона робить; `ENABLE_PAYMENT_LIVE` лишено швом на день, коли шлюз з'явиться. Так само полагодили `BookingV2` у `d1d52b4`: крок оплати прибрали, а не замінили. **Дорогою — чужа послуга.** `SERVICE_IDS = { sauna: 'svc_sauna', tub: 'svc_pool', breakfast: 'svc_breakfast' }` і `|| 'svc_sauna'`: `ServicesTab` віддає готелям справжній id послуги, який у мапу не влучає, тож віджет **кожного** іншого готелю мовчки питав `svc_sauna` — seed-id пілотного клієнта. Тепер `data-service` — це id, `data-kind` — лише вибір розкладки, а запасного варіанту немає: чесна відповідь на «яка послуга?» без вхідних — не чужа. Гостьова сторінка більше не викидає справжній id: у стані лежить `{ id, kind }`. Заодно зник `svc_${kind}` у заголовку шторки — він шукав `svc_tub`, хоч мапа казала `svc_pool`, тож купіль була без назви весь час. Промо-поле показується лише там, де є сайт: купон належить готелю і шукається через його сайт, а на гостьовій сторінці сайту немає — поле могло тільки відмовляти, і відмовляло. Гейт `check-embed-routes.mjs --strict`: кожен `/api/...` зі скриптів `public/widget/` звіряється з наявними маршрутами. Доведено зворотним двічі — повернув `/api/booking/promo` і зіпсував шлях у `collector.js`, гейт упав на обох. Коментарі він відрізняє від викликів, інакше падав би на власному поясненні. Проба лишається 53, зелена на обох двигунах.
| §2.9 завантаженість | (цей коміт) | «Завантаженість» рахувалася двома формулами під одним підписом, і розходились **обидві** половини дробу. Знаменник: звіт брав УСІ юніти — разом із pool-юнітом «Чорновик», за яким кімнати немає, і номерами, знятими з продажу, — тож готель на 10 номерів мав знаменник 12 і недосяжні 100 %. Чисельник: звіт рахував зайнятим `no_show` (броня, з якої ніхто не ночував, о 00:30 автоматично стає `no_show` — номер лишався б «зайнятим» назавжди), а дашборд не рахував `tentative`, хоча всі перевірки доступності в проєкті відкидають лише `cancelled`/`no_show`, тобто tentative тримає номер: вільним показувалось те, що продати вже не можна. Одну формулу винесено в `@core/occupancy-rate` — у `core`, а не в `modules/pricing`, бо завантаженість не ціна, а імпорт `pricing/domain` з `dashboard` і `reports` дав би два ПРОБОЇ меж за `check-boundaries`. Модуль приймає РЯДКИ, а не готові суми: інакше два хендлери й далі рахували б по-різному. Канон статусів — той самий `LIVE`, що в `day-sheets.repo.ts`. Дорогою: звіт порівнював рядок з `Date` із драйвера pg (`dateStr >= b.check_in`) — на Postgres це мовчки давало неправду. Доведено зворотним двічі: повернув «усі юніти» в знаменник — гейт упав (`12 !== 10`); прибрав `tentative` з чисельника — упав знову |
| §2.10 збори | (цей коміт) | `fees_taxes` існує від початку, квота її читає й додає до підсумку — а наповнював таблицю **лише demo-seed**: ні екрана, ні CRUD, ні секції у файлі готелю. Тобто в кожного реального клієнта міське мито й прибирання в квоті були нулем, і це мало вигляд «цей готель таких зборів не має». Портьє називає гостю суму саме з цього екрана; якщо мито не додалось, готель або доплачує його сам, або догадується на виїзді. Тепер збори заводяться секцією `fees` у файлі готелю — тим самим шляхом, що й усе інше: файл і пуш, без SSH. Застосування ідемпотентне (доведено другим прогоном), тип під CHECK перевіряється і в `check-hotels.mjs` (до деплою), і в applier (до INSERT); нульовий збір позначається окремо, бо він вставився б без заперечень і ніколи не потрапив би в квоту. Арифметику винесено в чистий `pricing/domain/fees.ts` із гейтом. Дорогою — дві речі в тому ж файлі: `per_person_per_night` рахував лише дорослих, а сусідній `per_person` — усіх (два поля з тим самим словом «person» рахували різні множини людей; готель зі збором «Сніданок, за особу за ніч» недобирав на кожній дитині), і `currency: 'CZK'` літералом — та сама помилка, що A1 у фоліо: німецький готель отримував квоту в кронах. Доведено зворотним: повернув `adults` — гейт упав. **І головне: власна проба знайшла те, чого в реєстрі не було** — `/api/pricing/quote` не перевіряв орендаря взагалі, тож на SQLite сусідній готель рахував квоту на чужий тип номера й бачив чужі ціни та збори (на Postgres ховала RLS). Фільтр тепер явний у SQL. Проба 54, зелена на обох двигунах |
| §2.11 дрібніше | (цей коміт) | Чотири окремі речі. **IDOR у `updateServiceOrder`**: дірка виявилась ширшою за опис — не лише статус, а complete/cancel/reopen чужого замовлення за одним id, зі списанням чужого `booked_count`. Жодна з двох таблиць не має власного `organization_id`, тож орендар береться тим самим шляхом, що в RLS-політиках; фільтр явний у SQL, бо на SQLite політик немає. Доведено зворотним: зі старими запитами атака відтворюється дослівно. У тому ж рядку знайдено `MAX(0, booked_count - 1)` — двоаргументний `max()` SQLite, якого в Postgres немає: **скасування замовлення зі слотом віддавало 500 на проді**. **Дати групової броні**: сервер їх не перевіряв узагалі, клієнт ховав відʼємну різницю за `Math.max(1, …)` — звідси «1 ніч». Перевірка тепер на сервері, і ночі рахує він сам зі своїх дат, а не бере з тіла. **Календар**: `if(res.ok)` без `else` — 422 з готовим поясненням («Неможливо заселити без повної оплати») екран просто викидав. **`test-email`**: власник міг надіслати довільний HTML на будь-яку адресу через SMTP готелю, підписаний готелем; тепер адресат — з переліку, який система вже знає (адреса сесії + `properties.email` цієї організації), а не з рядка запиту |
| A4/A5/A7 (частк.) | (цей коміт) + `6df83a7` | Турзбір — за налаштуванням обʼєкта, не за кодом: секція у формі броні (`6df83a7`) і вкладка «Збір» у модалці (цей коміт) зʼявляються лише коли `properties.city_tax_per_night` > 0 або бронь уже несе суму — ховати ненульові гроші не можна; формула — зі ставки обʼєкта, валюта — броні/організації, літералів «25» і «CZK» в обох місцях більше немає. Звіт city-tax пише валюту організації тим самим шляхом, що головний звіт (`useCurrentUser`, `6df83a7`). A7 закрито наполовину: курс 25.5 застосовується строго до `currency === 'CZK'` (раніше — до будь-якої не-EUR) і позначений коментарем як борг; у конфіг свідомо не переносився. Попутно: інлайн-зміна дат броні більше не масштабує стару середню ціну, а кличе справжню квоту `/api/pricing/quote`; квота без покриття ночей лишає стару суму і confirm каже це словами — нуль не підставляється |
| A3 (частк.) | `6df83a7` | Форма броні з порожнім полем вартості більше не створює бронь із тихим нулем: квота з `hasPricing false` або `missingDays > 0` зупиняє збереження з поясненням. Введена руками ціна не перевіряється — число власника священне |
| валюта броні | `3139d5f` | Валюта нової броні — `default_currency` організації, а не колонковий DEFAULT 'CZK': німецька бронь показувала «255 CZK» у списку і картці |
| D6 | `6df83a7` | Обмін кімната↔кімната вирізано: два послідовні PATCH не проходили серверний захист від перетинів, тож гілка завжди кидала «Обмін поки не підтримується» ПІСЛЯ того, як UI запросив перетягнути підсвіткою. Зайнята кімната — не ціль для drop (без підсвітки і тосту); перенос на вільну кімнату і в/з чорновика не зачеплено |
| маркетинг | (цей коміт) | Головна, /integrations, /modules/channels і секції product/module-channels більше не рекламують видалені Hostex/PriceLabs/Booking.com Connectivity/Teya; Booking.com лишився як OTA-джерело (iCal), бо це правда. Борг: джерело дизайну `design/alisio-pms.dc.html` досі містить старі тексти — наступний прогін `port-design.mjs` їх воскресить, чистити треба там же |
| §2.6 мертві виклики | (цей коміт) | Кнопка «Синхронізувати з Hostex» на календарі → `POST /api/hostex/sync`, маршруту немає з часу викорчовування Hostex. `res.json()` падав на HTML сторінки помилки, `catch` показував «Помилка синхронізації» — тобто кнопка виглядала як тимчасово зламана, а не як прибрана, і зрозуміти, що синхронізації просто не існує, не було як. Колонки `hostex_*` лишено: у них історичні дані вже завезених броней. Знайшов її не гейт — `check-embed-routes` стереже лише `public/widget/`. Тому написано `check-dead-fetch.mjs --strict`: кожен `/api/…` з коду екранів звіряється з маршрутами на диску (253). Доведено зворотним: повернув кнопку — гейт упав. Прогін по всьому коду виявив ще два мертві виклики, обидва внесені в іменований список гейта, бо вимагають рішення, а не правки рядка: `/api/finance/paid-services` (сторінка «Оплачені послуги» має посилання з `/app/finance`, маршруту немає) і `/api/guests/[id]/reservations` (мобільна картка гостя — список попередніх заїздів завжди порожній). Решта зі старого списку §2.6 уже закрита: `reconcile` і `ai/query` викликів не мають |
...
- **Ізоляція тримається на PG:** `check-isolation.mjs` — усі 50 тверджень green; `audit-routes` — 322/322 операторські хендлери під вартою; `check-await` — 0. Тобто фундамент RLS на проді цілий; `[SQLite]`-знахідки — про другий шар захисту й локальне середовище.
