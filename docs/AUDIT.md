...
- **Ізоляція тримається на PG:** `check-isolation.mjs` — усі 46 тверджень green; `audit-routes` — 322/322 операторські хендлери під вартою; `check-await` — 0. Тобто фундамент RLS на проді цілий; `[SQLite]`-знахідки — про другий шар захисту й локальне середовище.
