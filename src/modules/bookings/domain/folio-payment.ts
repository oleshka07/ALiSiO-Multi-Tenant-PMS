 ...
 *   - `unpaid`  — є що платити, а грошей немає (у тому числі після
 *                 зустрічного рядка, який зняв помилковий платіж);
 *   - `null`    — книга порожня зовсім: нічого не нараховано і нічого не
 *                 сплачено, тож слово броні лишається як стояло.
 *
...
export function statusFromFolio(summary: SettleSummary | null | undefined): 'paid' | 'partial' | 'unpaid' | null {
  ...
  // Грошей немає, але Є ЩО ПЛАТИТИ — це не мовчання, це «не оплачено».
  ...
  if (!(paid > 0)) return charged > 0 ? 'unpaid' : null;
