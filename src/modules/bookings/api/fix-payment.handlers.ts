...
export async function fixServiceOrderPayment(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (orderId || orderIds) {
      ...
      for (const id of ids) {
        const r1 = await sql.run("UPDATE service_orders SET payment_status = 'paid', status = 'confirmed' WHERE id = ? AND payment_status IN ('pending', 'none')", [id]);
        const r2 = await sql.run("UPDATE booking_service_orders SET payment_status = 'paid', status = 'confirmed' WHERE id = ? AND payment_status IN ('pending', 'none')", [id]);
    ...
    if (reservationId && markAllPaid) {
      const r1 = await sql.run("UPDATE service_orders SET payment_status = 'paid', status = 'confirmed' WHERE reservation_id = ? AND payment_status IN ('pending', 'none')", [reservationId]);
      const r2 = await sql.run("UPDATE booking_service_orders SET payment_status = 'paid', status = 'confirmed' WHERE reservation_id = ? AND payment_status IN ('pending', 'none')", [reservationId]);
      results.push({ reservationId, so: r1.changes, bso: r2.changes });
    }

    // Case 3: Fix by Teya session ID (retroactive webhook replay)
    if (sessionId) {
      const r1 = await sql.run("UPDATE service_orders SET payment_status = 'paid', status = 'confirmed' WHERE payment_id = ? AND payment_status IN ('pending', 'none')", [sessionId]);
      const r2 = await sql.run("UPDATE booking_service_orders SET payment_status = 'paid', status = 'confirmed' WHERE payment_id = ? AND payment_status IN ('pending', 'none')", [sessionId]);
