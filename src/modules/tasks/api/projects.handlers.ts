...
export async function getProject(_request: NextRequest, context: IdParams): Promise<NextResponse> {
  try {
    ...
    console.error('GET /api/tasks/projects/:id error:', error);
    return NextResponse.json({ error: 'Failed to fetch project' }, { status: 500 });
  }
}

export async function updateProject(request: NextRequest, context: IdParams): Promise<NextResponse> {
  try {
    const { id } = await context.params;
    const body = await request.json();
    ...
    console.error('PATCH /api/tasks/projects/:id error:', error);
