...
async function countChildren(categoryId: string): Promise<number> {
  const sql = getSql();
  const row = await sql.row<any>("SELECT COUNT(*) AS n FROM expense_categories WHERE parent_id = ?", [categoryId]) as { n: number };
  return row.n;
}
...
export async function createCategory(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const maxOrder = await sql.row<any>(
      "SELECT COALESCE(MAX(sort_order), 0) AS mx FROM expense_categories WHERE organization_id = ? AND parent_id IS NOT DISTINCT FROM ?",
      [orgId, parent_id],
    ) as { mx: number };
...
export async function updateCategory(
  ...
  try {
    ...
    if (op_type !== undefined) {
      ...
      await sql.run("UPDATE expense_categories SET op_type = ? WHERE parent_id = ?", [op_type, id]);
    }
    if (classifier !== undefined) {
      ...
      await sql.run("UPDATE expense_categories SET classifier = ? WHERE parent_id = ?", [classifier, id]);
    }
...
export async function deleteCategory(
  ...
  try {
    ...
    const expenseCount = await countLinkedExpenseOps(id);
    if (expenseCount > 0) {
      return NextResponse.json(
