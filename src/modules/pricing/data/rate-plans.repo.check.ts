...
try {
  ...
  // Читання — очима Б, не А (рецензія раунду 8, Р8.11).
  //
  // «У Б нічого не зʼявилось» під контекстом А доводить рівно нічого: під
  // роллю застосунку політика і так не показує А чужих рядків, тож нуль там
  // безумовний — твердження було б зеленим і тоді, коли тариф у Б справді
  // ліг. Питати треба того, у кого шукаємо: контекст Б бачить свої рядки, і
  // якщо серед них зʼявився чужий тариф, це видно.
  assert.strictEqual(
    (await runWithOrganization(B, () => sql.rows('SELECT id FROM rate_plans WHERE property_id = ?', [PROP(B)]))).length,
    0, 'у Б зʼявився тариф, якого він не створював');
