...
try {
  await runWithOrganization(A, async () => {
    ...
    // ── Режим ціни задає опції (Блок 2.2; уточнено 05.09, розділ A п.3) ──
    //
    ...
    // кожну кількість ДОРОСЛИХ, для якої є ДЖЕРЕЛО ЦІНИ: базова заселеність
    // (її цінує сам тариф) і ті кількості, які знає матриця на цьому типі.
    // До 05.09 опція заводилась на кожну кількість до місткості, а ціну без
    // рядка матриці батчер брав рівною ціні тарифу — четверо дешевше за
    // трьох (Ц26 (б), скасовано). Опція, яку нема чим оцінити, у каталог не
    // йде: у вендора вона стояла б закритою назавжди.
    ...
    assert.deepStrictEqual(pp.unitTypes.find((u) => u.code === 'STU')!.occupancies, [2],
      '«за особу» — опція лише на кількості з джерелом ціни: базову і ті, що знає матриця; студія знає лише двох — опція на одного пішла б у вендора без ціни');
    // Двомісний у BAR: матриця знає 1 і 2, база 2 — обидві опції з джерелом.
