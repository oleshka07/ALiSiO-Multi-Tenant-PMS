...
let shared: ChannexRateLimiter | null = null;

/**
 * Один обмежувач на процес: бюджетом обʼєкта володіє ОБʼЄКТ, не прохід.
 *
 * Клієнт будується на кожен прохід (каталог, стрічка, розсилка), і обмежувач
 * у ньому знав би лише власні виклики: ручний «повторити» і черговий прохід
 * крона в одну хвилину зʼїли б бюджет обʼєкта разом і не помітили б. Тому
 * дефолт клієнта — цей екземпляр, ключований зʼєднанням (И10). Межа: один
 * процес; два контейнери одного середовища ділили б бюджет наосліп — коли
 * таке зʼявиться, стан переїде в базу (`cm_connections`), не в память.
 */
export function sharedChannexLimiter(): ChannexRateLimiter {
  shared ??= new ChannexRateLimiter();
  return shared;
}
