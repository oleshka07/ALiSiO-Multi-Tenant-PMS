 ...
 */
/*
 * Власного списку мов тут НЕМАЄ.
 *
 * Стояв `CATALOG_LANGUAGES = ['uk','en','de',…]` — сьомий список у проєкті.
 * Він збігався з реєстром і множиною, і порядком, тож нічого не ламав рівно
 * доти, доки порядок реєстру не змінився: копія, «випадково така сама»,
 * розходиться з оригіналом мовчки і саме тоді, коли оригінал міняють.
 *
 * Хто питає «якими мовами має бути заповнений словник» — питає `LANGUAGE_CODES`
 * напряму. Одна назва менше, один список менше.
 */
