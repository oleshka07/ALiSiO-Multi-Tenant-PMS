...
try {
  ...
  // ── А тепер БЕЗ рядка обʼєкта взагалі (рецензія 6, п. 3.2) ────────────
  //
  // Четвертий шлях, якого фікстура не бачила: `property_guest_config` — це
  // рядок, а не обовʼязок, і готель може його ніколи не заповнити. У цій
  // гілці резолвер повертав `{ ...unitTypeConfig }` цілком, тобто правило
  // «рівень береться цілим» не діяло взагалі: тип із самою назвою віддавав
  // гостю назву без пароля.
  //
  // Фікстура тут спеціально КЛАДЕ обʼєкт останнім кроком назад — інакше
  // наступний прогін почав би з іншого стану, ніж попередні сцени.
  {
    await runWithOrganization(ORG, async () => {
      await sql.run('DELETE FROM property_guest_config WHERE property_id = ?', [PROP]);
    });
    await setUnit({ wifi_network: null, wifi_password: null });
    await setType({ wifi_network: 'TYPE-NET-ONLY' });
    const cfg = await read() as any;
    assert.ok(!cfg.wifi_network,
      'без рядка обʼєкта половина мережі типу поїхала гостю: назва без пароля — це те саме підключення, якого не буде (О2)');
    assert.ok(!cfg.wifi_password, 'пароль зʼявився там, де назви немає');
    console.log('  ok  правило діє й тоді, коли обʼєкт не заповнено взагалі');
  }
