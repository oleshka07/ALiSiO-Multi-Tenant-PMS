 ...
 *
 * `t` — транзакція викликача: заведення готелю робить це всередині своєї
 * (інваріант 11), а екран — сам по собі.
 */
 ...
 *
 * Заведення нового готелю сіє каталог саме там, де це видно
 * (`provisionOrganization`). Але готелі, які вже працюють, через ту функцію
 * більше не проходять НІКОЛИ, а міграція, яка сіяла б за них, мусила б
 * вгадати мову кожного просто в SQL. Тому порожній каталог досівається при
 * першому читанні екрана: мова береться з організації, вставка ідемпотентна
...
export async function ensureAmenityCatalog(organizationId: string): Promise<void> {
  ...
  await seedAmenityCatalog(organizationId, String(org?.language ?? 'uk'));
}
