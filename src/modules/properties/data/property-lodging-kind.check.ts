...
await runWithOrganization(ORG, async () => {
  ...
  // Правка з ПОРОЖНІМ полем — і це інша сцена, не варіація попередньої.
  //
  // Перша редакція перевіряла лише «поля немає» і була зеленою на коді, який
  // стирав рід: форма обʼєкта шле `{...propForm}` ЦІЛКОМ, тобто
  // `property_type: ''` приїжджає в КОЖНОМУ PATCH — не «іноді», а завжди.
  // Порожній рядок ставав `null`, `null !== undefined`, і цикл писав
  // `property_type = NULL` поверх названого роду. Тобто єдиний живий шлях
  // правки обʼєкта знімав вибір, а екран показував «Обʼєкт оновлено!»
  // (рецензія раунду 20, П1).
  //
  // Тіло навмисно таке саме, як шле форма: рід порожній, поруч інше поле,
  // яке справді міняється. Без другого поля сцена не розрізняла б «поле
  // знято з патча» і «патч не застосувався взагалі».
  const asForm = await repo.updateProperty(ORG, made.id, {
    property_type: '', phone: '+380111111111',
  });
  assert.strictEqual(asForm?.property_type, 'camping',
    `правка з порожнім полем стерла рід житла (${asForm?.property_type ?? 'null'}) — `
    + 'саме так шле форма обʼєкта, у кожному збереженні');
  assert.strictEqual(asForm?.phone, '+380111111111',
    'решта патча не застосувалась — сцена нижче нічого б не розрізняла');
