 ...
 *
 * ── Відкрите питання власника ───────────────────────────────────────────
 *
 * Роль `receptionist` СЬОГОДНІ не має `manage_documents` (див.
 * `ROLE_PERMISSIONS` у `core/auth/permissions.ts`) — тобто рецепція не може
 * виписати рахунок на виїзді ні до цієї зміни, ні після. Це не наслідок
 * розділення модулів, це стан рольової матриці, і змінювати його мовчки
 * тут було б неправильно: «хто має право видати документ від імені готелю»
 * — рішення власника, не рефакторингу.
 *
 * Якщо відповідь «рецепція має», у `receptionist` додається
 * `manage_documents` і `nav:documents` — і більше нічого не треба.
 */
