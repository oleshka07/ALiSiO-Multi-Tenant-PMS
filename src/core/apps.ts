...
export interface OwnAppManifest extends ManifestBase {
   ...
   *
   * Точніше — «код застосунку існує, і його МОЖНА ввімкнути»: саме це поле
   * вирішує, чи екран покаже вимикач (`live && feature`), і саме тому
   * `kiosk` і `guest_app` тут `true`, хоч вендора в них немає взагалі —
   * поверхня їхня власна. `false` означає картку «скоро» з кнопкою «хочу»,
   * і тоді вимикача немає, отже й ключа реєстру бути не може.
   */
  live: boolean;
}
...
export const APPS: readonly AppManifest[] = [
  ...
  // Другий застосунок рівня 3 — і перший роду `device` (Блок «Кіоск»,
  // docs/tasks/2026-09-10-block-kiosk.md). Платний (З1): це термінал у холі,
  // а не частина PMS. `scope: 'property'` — стан звʼязку належить БУДИНКУ:
  // екран стоїть у холі конкретного корпусу, і «термінал не озивається» це
  // питання про той корпус, а не про рахунок.
  //
  // Полів ключів немає — з тієї самої причини, що в Winhotel: тут ключ не
  // вставляють, а видають. Адмін робить шестизначний код парування, термінал
  // обмінює його на довгий токен, і в базу лягає лише хеш
  // (`src/apps/kiosk/data/device-token.ts`).
  {
    id: 'kiosk',
    kind: 'device',
    label: 'Кіоск самообслуговування',
    feature: 'kiosk',
    fields: [],
  ...
  // Фіскалізація України (ПРРО). Рід той самий, що в fiskaly, і `scope`
  // теж: каса стоїть у БУДИНКУ, не в рахунку (INC-029, закон З11).
  //
  // `live: true` при тому, що ПРОВАЙДЕРА ще не обрано — і це не описка.
  // Поле вирішує, чи екран дасть вимикач, а вимикати тут є що: ключ стереже
  // касу (UA-обʼєкт без нього не приймає готівку взагалі), екран модуля
  // працює, журнал пишеться, драйвер `test` відповідає. Немає рівно одного —
  // драйвера, що говорить із чужим реєстратором, бо його не обрано
  // (чекпоінт власника, docs/research/prro-providers.md). Картка «скоро»
  // означала б протилежне: ключ без вимикача, тобто варту, яку неможливо
  // ввімкнути.
  //
  // Полів ключів немає з тієї ж причини: чиї це будуть ключі й скільки їх —
  // питання до провайдера, якого нема. Реквізити каси (касир, фіскальний
  // номер реєстратора) живуть на екрані модуля, бо вони НЕ секрети.
  {
    id: 'fiscal_ua',
    kind: 'fiscal',
    label: 'Фіскалізація України (ПРРО)',
    feature: 'fiscal_ua',
    fields: [],
    where: 'Реквізити каси й вибір драйвера — Налаштування → Фіскалізація України',
    live: true,
    pricing: 'paid',
    scope: 'property',
  },
