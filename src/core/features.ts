...
export const FEATURE_SPEC = {
  ...
  // Українська фіскалізація (ПРРО). Поки вимкнена, UA-обʼєкт НЕ може
  // записати готівку чи карту-на-рецепції — з тієї самої причини, що й
  // `fiscal_de` вище: PMS, яка приймає готівку без зареєстрованої каси, і є
  // незареєстрованою касою. Вісь — країна ОБʼЄКТА, не мова оператора і не
  // валюта (`invoicing/domain/fiscal/till-jurisdiction.ts`).
  //
  // OFF, платно — П15 (05.09.2026). Міграції з явним `enabled = FALSE` тут
  // НЕМАЄ і не треба: рядок дописують лише тоді, коли дефолт міняється з ON
  // на OFF і наявні готелі інакше мовчки втратили б функцію (0045, 0049,
  // 0065). Новий ключ із дефолтом OFF нічого ні в кого не забирає — його
  // просто ні в кого ще немає.
  fiscal_ua: { label: 'Фіскалізація України (ПРРО)', on: OFF, kind: APP },
  ...
  // Гостьовий застосунок — сторінка, куди веде QR на склі (12.09.2026).
  // OFF з тієї ж причини, що кіоск, і ще з однієї, важливішої: поки
  // вимкнено, ПУБЛІЧНА сторінка `/stay/<ключ>` відповідає 404, навіть якщо
  // ключ у обʼєкта вже виписаний. Прапорець без варти — перемикач-обманка
  // (П5), а тут за ним стоїть поверхня, з якої видно назву готелю, його
  // вільні номери й ціни.
  guest_app: { label: 'Гостьовий застосунок', on: OFF, kind: APP },
