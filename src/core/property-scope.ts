...
/**
 * Обʼєкт для НОВОГО рядка конфігурації фінансів (INC-038, Д54).
 *
 * Те саме, що `scopedPropertyId`, з однією поправкою: у рахунку з ОДНИМ
 * обʼєктом повертається `null`, тобто «на весь рахунок».
 *
 * Причина не в акуратності, а в тому, що інакше ламається. Провайдер області
 * навмисно робить єдиний обʼєкт областю («обирати нема з чого»), тож готель з
 * одним будинком шле `property_id=<його id>` на кожному екрані. Прив'язавши до
 * нього ставки ПДВ, ми лишили б без ставок кожен документ БЕЗ обʼєкта —
 * рахунок компанії, подію, — і виписування відмовляло б названою відмовою там,
 * де оператор щойно все налаштував.
 *
 * З двох рівнозначних варіантів (у рахунку з одним будинком «мій будинок» і
 * «весь рахунок» — це одна й та сама множина документів) `NULL` кращий ще й
 * тим, що переживає появу другого будинку: за Д54 спільний набір застосовується
 * до будинку тієї самої країни, тобто новий обʼєкт успадкує налаштування, а не
 * почне з порожнечі.
 *
 * ЧИТАННЯ це не стосується: список і далі звужується областю, інакше рядок,
 * заведений до появи другого будинку, зник би з екрана.
 */
export async function configPropertyId(scope: PropertyScope): Promise<string | null> {
  if (scope.kind !== 'one') return null;
  const organizationId = await requireOrganizationId();
  const rows = await getSql().rows<{ id: string }>(
    'SELECT id FROM properties WHERE organization_id = ? LIMIT 2',
    [organizationId],
  );
  return rows.length > 1 ? scope.id : null;
}
