...
export async function provisionOrganization(input: NewOrganization): Promise<ProvisionedOrganization> {
  ...
  await runWithOrganization(organizationId, async () => await sql.tx(async (t) => {
    ...
    // ── Довідник способів оплати: чотири стандартні рядки (Д61) ──────────
    //
    // Міграція 0141 засіяла їх КОЖНІЙ організації, яка існувала на той
    // момент, — і на цьому все. Тобто вона закрила минуле й не закрила
    // майбутнє: готель, заведений ПІСЛЯ неї, дістав би порожній довідник.
    //
    // Це дзеркало INC-025/INC-028, де було навпаки — засів для нових без
    // зворотного заповнення для наявних (Д67). Той бік коштував власникові
    // неприйнятої готівки; цей поки латентний (HTTP-шлях `method_id` ще не
    // передає), і саме тому його легко не помітити: на SQLite `db.ts` сіє
    // ці рядки кожному готелю при кожному старті, тож у розробника довідник
    // ЗАВЖДИ є, а на Postgres його немає. Класична пара, від якої
    // застерігає AGENTS §7: помилка виглядає як «функція зникла».
    //
    // `code` = `kind`, `name` — NULL: міграція не вигадує слів жодною мовою,
    // і тут так само (екран показує переклад класу, поки готель не написав
    // своє). Список чотирьох класів навмисно повторює CHECK платіжки — це
    // словник продукту, а не дані клієнта (інваріант 20).
    for (const [code, position] of [['cash', 0], ['card_terminal', 1], ['transfer', 2], ['voucher', 3]] as const) {
      await t.run(`
        INSERT INTO fin_payment_methods (id, organization_id, code, kind, position)
        VALUES (?, ?, ?, ?, ?)
      `, [`pm_${crypto.randomBytes(8).toString('hex')}`, organizationId, code, code, position]);
    }
