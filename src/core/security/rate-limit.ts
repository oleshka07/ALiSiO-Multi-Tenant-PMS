...
export type RateLimitedAction = 'service_order' | 'registration' | 'payment_request'
  ...
  | 'kiosk_device'
  // Телефон гостя, який відсканував QR у холі. Ключ — IP, і ліміт СУВОРІШИЙ за
  // термінальний: біля термінала стоїть людина в холі, а сюди стукають із
  // будь-якого телефона в місті — токен передачі публічний рівно тому, що його
  // фотографують з екрана.
  | 'kiosk_handoff';
