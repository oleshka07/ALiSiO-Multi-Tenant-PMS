...
export type RateLimitedAction = 'service_order' | 'registration' | 'payment_request'
  ...
  | 'kiosk_device'
  // Пошук своєї броні з ТЕЛЕФОНА (гостьовий застосунок). Ключ — IP, і саме
  // IP: гість ще нічим не доводить, хто він, а маршрут шукає в персональних
  // даних. Без ліквідного ліміту пара «телефон + імʼя» підбирається
  // перебором, бо вікно ±1 день робить множину маленькою — тією самою
  // властивістю, яка робить пошук безпечним для чесного гостя.
  | 'guest_lookup';
