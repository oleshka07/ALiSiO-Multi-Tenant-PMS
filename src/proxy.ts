...
const PUBLIC_PREFIXES = [
  ...
  //
  // Під цим префіксом лежить ЛИШЕ те, що відповідає терміналу. Картка
  // застосунку живе там, де всі інші картки, — `/api/settings/apps/kiosk/`,
  // тобто в звичайному охоронюваному контурі. Перша редакція тримала її тут
  ...
  '/api/apps/kiosk/',
  // Гостьовий застосунок (QR на склі): пошук своєї броні, вільні номери з
  // цінами, бронювання зі строком і заявка «я щойно забронював у готелю».
  // Сесії немає за визначенням — орендаря називає ключ у тілі запиту, той
  // самий, що в адресі сторінки. Чотири властивості, які роблять публічний
  // пошук у персональних даних прийнятним, — у domain/lookup.ts застосунку;
  // бронь без прийнятих умов не створюється — domain/consents.ts (КІ29).
  '/api/apps/guest/',
