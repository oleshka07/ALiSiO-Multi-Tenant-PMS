...
const PUBLIC_PREFIXES = [
  ...
  //
  // Сюди ж потрапляє `/kiosk/go/<токен>` — сторінка, яку відкриває ТЕЛЕФОН
  // гостя після QR у холі. Окремого рядка для неї немає навмисно: перелік
  // звіряється через `startsWith`, тож другий запис був би мертвим і при
  // цьому виглядав би як окреме рішення. Право там доводить токен передачі в
  // самій адресі (`readHandoff`) — той самий рід перепустки, що
  // `/guest/<токен>` нижче.
  '/kiosk',
