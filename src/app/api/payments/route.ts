...
export const POST = withPermission('manage_payments', async (
  ...
  try {
    ...
    if (CASH_METHODS.has(method)) {
      ...
      // Слід у журналі броні лишає САМ МІСТОК (`noteMoneyOnBooking`), і це
      // не переставляння рядків: доти його писав цей маршрут, тобто оплата з
      // екрана фоліо не лишала на картці нічого, хоч гроші рухались так само.
      // Слід лишає той, хто рухає гроші, а не той, хто про це попросив —
      // інакше «одні двері» правдиві на три книги з чотирьох (Д83).
    ...
    // ── Повернення позначкою не буває (П16, ревізія 16.09.2026) ────────
    //
    // Для ПРИХОДУ позначка чесна: гроші ще в дорозі, і рядок зʼявиться з
    // випискою. Для ПОВЕРНЕННЯ все навпаки — гроші вже пішли з готелю, і
    // «позначка» означала б, що видача грошей не записана ніде: ні в касі, ні
    // в рахунку гостя, ні у звіті. Названа відмова замість тихого рядка.
    if (type === 'refund') {
      return NextResponse.json({
        error: 'Повернення не записується позначкою: гроші вже пішли з каси. '
          + 'Оберіть «Готівка» — тоді воно ляже і в касу, і в рахунок гостя, — '
          + 'або проведіть витрату у Фінансах, якщо гроші повертає банк.',
      }, { status: 409 });
    }
