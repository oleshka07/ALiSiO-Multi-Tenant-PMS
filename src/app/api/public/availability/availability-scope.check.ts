 ...
 *
 * Вікно запиту — ТИХИЙ місяць фікстури (`quietMonth`), у якому вона за
 * побудовою не має жодної броні: тоді число зайнятих дат — це рівно те, що
 * засіяла перевірка, і «3» не треба виводити відніманням.
 *
...
/**
 * Вікно — ТИХИЙ місяць фікстури, а не літерал.
 *
 * Тут стояв листопад із приміткою «навмисно не перетинається з бронями
 * фікстури (вересень)». Примітка була правдою, доки обидва місяці були
 * літералами: щойно фікстура переїхала на відносні дати, ЇЇ місяцем став той
 * самий листопад, і три твердження про зайняті дати почервоніли. Тепер
 * непересічність обчислена й стверджена самою фікстурою
 * (`assertQuietMonthIsQuiet`), а не обіцяна коментарем.
 */
const WINDOW_FROM = fx.quietFrom;
const WINDOW_TO = fx.quietTo;

/** Бронь на `nights` ночей від `day` числа ТИХОГО місяця — щоб число ночей було видно очима. */
const stay = async (
  ...
  await sql.run(
    ...
    [id, organizationId, propertyId, unitId, unitTypeId, guestId,
      `${fx.quietMonth}-${pad(day)}`, `${fx.quietMonth}-${pad(day + nights)}`, nights],
  );
};
