...
#
# ПОПРАВКА ТОГО Ж ДНЯ. Перша редакція цієї перевірки кричала «інше дерево,
# спільного предка немає» — і сама була прикладом того, що ловить. Клон
# веб-сесії **обрізаний** (`git rev-parse --is-shallow-repository` → true,
# межі в `.git/shallow`), а в обрізаному клоні merge-base не знаходить
# нічого просто тому, що історії немає локально. Я на цьому й попався:
# назвав межу обрізання коренем дерева і зробив висновок, що два продукти
# не мають спільної історії. Насправді корінь у них один, гілка була на 411
# комітів позаду, і жодного --allow-unrelated-histories не треба.
#
# Тому: у обрізаному клоні гучної тривоги НЕ БУВАЄ. Хибна тривога, яку всі
# вчаться ігнорувати, гірша за відсутню — це вже записано вище про merge-
# коміти, і тут те саме правило.
BRANCH="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo '?')"
SHALLOW="$(git rev-parse --is-shallow-repository 2>/dev/null || echo unknown)"
if git rev-parse --verify --quiet origin/main >/dev/null 2>&1; then
  BASE="$(git merge-base HEAD origin/main 2>/dev/null || true)"
  if [ -z "$BASE" ] && [ "$SHALLOW" = "true" ]; then
    echo "(клон обрізаний: спорідненість із main перевірити не можна."
    echo " Схему читай як 'git show origin/main:src/lib/db.ts', не з робочої копії.)"
  elif [ -z "$BASE" ]; then
    echo ""
    echo "!! ЦЯ ГІЛКА НЕ МАЄ СПІЛЬНОЇ ІСТОРІЇ З origin/main, і клон ПОВНИЙ —"
    echo "!! тобто це справді інше дерево, а не артефакт обрізання."
    echo "!! Усе, що тут виміряно, продукту може не стосуватися."
    ...
    # про продукт.
    # Відставання саме по собі нормальне. Небезпечний ВІК точки розгалуження:
    # у обрізаному клоні число занижене, але вік рахується від самої точки і
    # лишається чесним — саме він і ловить те, що сталося насправді.
    ...
    if [ "${BEHIND_MAIN:-0}" != "0" ]; then
      ...
      [ "$SHALLOW" = "true" ] && APPROX=" (клон обрізаний, число занижене)" || APPROX=""
      echo "Відгалужена від main ${FORK_AGE} дн. тому; main відтоді пішов на ${BEHIND_MAIN} коміт(ів)${APPROX}."
      if [ "${FORK_AGE:-0}" -gt 7 ]; then
        echo "!! Точка розгалуження старша за тиждень — у цьому проєкті це вже"
        echo "!! інша схема. Читай 'git show origin/main:src/lib/db.ts', а не"
        echo "!! файл із робочої копії: він тут застарілий."
      fi
    fi
  fi
fi
