...
# 2026-08-28 (purge-tenant.sh), і список тут суворий аж до слова «сауна» —
# бо реальних клієнтів у базі нуль, тож будь-який збіг означає, що спадок
# повернувся крізь дані. Щойно зʼявиться готель зі СВОЄЮ сауною — звузьте
# список до формулювань першого клієнта (вузькі зразки — у FORBIDDEN_IN_CODE
# гейта check-no-tenant-names.mjs).
LEGACY_MARKERS='сауна|saun[ay]|купіль|купел|káď|kádě|глемпінг|glamping|карпатськ|kemp[ -]?carlsbad|kempcarlsbad|luhačovic|лугачовіце|buchlov|бухлов|lešná|лешна|будов[аиуі] [fd]'
LEGACY_HITS="$(zcat "$DUMP" | grep -aciE "$LEGACY_MARKERS" || true)"
if [ "${LEGACY_HITS:-0}" -gt 0 ]; then
  note_fail "у дампі $LEGACY_HITS рядків зі спадком першого клієнта (сауна/купіль/глемпінг/…) — purge-tenant не завершився або спадок повернувся крізь дані"
