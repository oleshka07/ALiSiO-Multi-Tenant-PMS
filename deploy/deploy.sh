...
for i in $(seq 1 45); do
  ...
  case "$CODE" in
    401|400)
      ...
      # І ОДРАЗУ звірка: «поставив» і «стоїть» — різні твердження.
      #
      # 19.09.2026 у живому crontab прода стояло ТРИ рядки з девʼяти: бракувало
      # channels-pull, channels-publish, guest-app-holds і kiosk-day, бо їх
      # додали після 10.09, а прод з того дня не деплоївся. Броні з каналів не
      # приїжджали, ціни туди не їхали, холди не знімались — і жодна з цих тиш
      # не має вигляду помилки. Деплой при цьому вважався успішним.
      #
      # Провал звірки деплой не валить (застосунок уже живий і здоровий), але
      # мовчати про неї не можна: саме мовчання і коштувало девʼяти днів.
      ./deploy/check-cron.sh "$ENV_NAME" \
        || echo "!! розклад розійшовся зі скриптом — див. вище, які роботи НЕ виконуються" >&2
      exit $HOTELS_OK
      ;;
    500|502|503)
