...
echo "==> cron (розклад: «поставлено» і «стоїть» — різні твердження)"
# 19.09.2026 тут було три рядки з девʼяти, і саме цього рядка бракувало, щоб
# це побачити: розклад ставить деплой прода, а прод не деплоївся з 10.09.
# Провал не валить status.sh — він показує стан, а не виносить присуд.
./deploy/check-cron.sh "$ENV_NAME" 2>&1 | sed 's/^/    /' || true
