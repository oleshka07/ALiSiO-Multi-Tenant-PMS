...
#   2. Скільки броней мають ДВА номери на ТІ САМІ гроші. Це саме пара
#      «позначка оплати» (legacy, folio_id IS NULL) + «рахунок гостя», або
#      дві позначки: перша виписується на `total_price`, друга — на рядки
#      рахунку, тобто на ту саму стою двічі. Виправляється лише сторно.
#
#      ДВА документи з ОДНОГО рахунку гостя сюди не входять і входити не
#      мусять: рахунок законно виставляє другий номер на пізніші нарахування
#      (виїхав, зайшов у бар). Перша редакція цього скрипта рахувала будь-які
#      2+ і називала їх «ті самі гроші» — виміряно прогоном на стенді, де
#      вона так назвала законну пару власного гейта. Числу, яке вимагає
#      сторно, не можна бути приблизним.
...
-- 2. ДВА НОМЕРИ НА ТІ САМІ ГРОШІ ───────────────────────────────────────────
--
-- Ознака — участь ПОЗНАЧКИ ОПЛАТИ (`folio_id IS NULL`): вона виписується на
-- `total_price`, тобто на всю стою. Поруч із рахунком гостя, виставленим на
-- рядки тієї ж стої, це та сама сума під двома номерами. Дві позначки — те
-- саме, тільки без фоліо.
SELECT count(*) AS "два номери на ті самі гроші" FROM (
  SELECT reservation_id
    FROM invoices
   WHERE reservation_id IS NOT NULL AND status <> 'cancelled'
   GROUP BY reservation_id
  HAVING count(*) > 1
     AND count(*) FILTER (WHERE folio_id IS NULL) > 0
) x;
...
HAVING count(*) > 1
   AND count(*) FILTER (WHERE i.folio_id IS NULL) > 0
 ORDER BY 1
 LIMIT 50;

-- Для повноти — скільки броней мають 2+ документи ВСЬОГО. Це не присуд:
-- рахунок гостя законно виставляє другий номер на пізніші нарахування.
SELECT count(*) AS "2+ документи всього (не присуд)" FROM (
  SELECT reservation_id FROM invoices
   WHERE reservation_id IS NOT NULL AND status <> 'cancelled'
