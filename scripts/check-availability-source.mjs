...
const EXEMPT = new Map([
  ...
  // Сцена обмеження бази (INC-045) — вона не питає «чи вільно», вона ЧИТАЄ
  // НАЗАД, що лишилось у таблиці після двох паралельних записів. Читати це
  // через `@properties` не можна за призначенням: доказ у тому, щоб подивитись
  // на рядки повз усі наші двері (AGENTS §27). Пере­писати запит так, щоб він
  // просто не збігся з візерунком, було б підгонкою коду під гейт — сьомий
  // підвид §3.2.1; тому запис тут, з причиною.
  ['src/modules/bookings/data/double-booking.check.ts',
    'сцена INC-045: читання назад після паралельного запису, а не розрахунок наявності'],
]);
