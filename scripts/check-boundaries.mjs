...
const BASELINE = {
  ...
  // 10 → 2 (16.09.2026, ревізія платіжних шляхів). Стеля була ЗАСТАРІЛА, і це
  // перевірено: з відкоченими змінами цієї сесії гейт дає ті самі 2. Тобто
  // вісім пробоїв пішли раніше, разом із переїздом фактурування в
  // `@invoicing`, а стелю за ними не опустили — храповик стояв із запасом у
  // вісім місць, тобто НЕ тримав нічого. Те, що лишилось, — два читання
  // `fin_operation_audit` (маршрут історії фінансів і гейт ізоляції), і
  // лікуються вони дверима фасаду, а не переїздом файла.
  finance: 2,
