...
const BASELINE = {
  ...
  // 3 → 4 (09.09.2026, Блок «Застосунки»): `src/core/apps.check.ts` ВИКОНУЄ
  // клієнт fiskaly (`data/fiskaly-sign-de.ts`) із підставленим fetch, щоб
  // довести, що стан звʼязку пишеться на обох гілках із текстом відмови
  // (§5.6 задачі — «фікстура з двома результатами для fiskaly-клієнта»).
  // Дверей у фасаді `@invoicing` для клієнта TSE немає, а фасад — тека
  // сесії 1; один експорт там (`fiskalyDevice`) поверне стелю на 3. Це
  // гейт, не робочий код: у продукті модуль відкривають лише фасади.
  // 4 → 5 (09.09.2026, 3.8 того ж блоку): `api/settings/apps/_handlers.ts`
  // кличе `fiskalyConnect` (підключення TSE, quickstart 2–3) з того самого
  // `data/fiskaly-sign-de.ts` — контролер поставив дію на картку застосунку,
  // а дверей у фасаді для клієнта TSE досі немає. Один експорт у
  // `@invoicing` (тека сесії 1) повертає обидва рядки: стеля назад на 3.
  invoicing: 5,
