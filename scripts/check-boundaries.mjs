...
const BASELINE = {
  ...
  // 3 → 4 (10.09.2026, задача 12): `scripts/count-payer-folios.mjs` читає
  // `fin_folios` власним SQL, і фасад `@invoicing` тут НЕ двері — не з
  // недогляду, а за побудовою. Скрипт питає про БАЗУ, а не про орендаря:
  // «чи немає в цій базі рахунків без броні, які не є фоліо платника»,
  // одним числом по всіх організаціях, перед переїздом клієнта. Кожен читач
  // модуля навпаки починається з `requireOrganizationId()` і живе під
  // політиками — тобто відповісти на це питання він не може за означенням.
  // Це той самий рід, що `check-deployed-db.mjs` і `deploy/list-tenants.sh`:
  // інструмент оператора, який навмисно не піднімає застосунок (тому й
  // самий лише `pg`, і тому він проходить `check-entry-imports`).
  // Перенести його в `.sh`, щоб гейт не бачив, було б Р14.2 — правкою коду
  // під гейт замість правки в гейті.
  // 4 → 3 (10.09.2026, Блок «Застосунки», рецензія 1): клієнт TSE відкрито
  // одним рядком фасаду `@invoicing` — екран застосунків і його гейт більше
  // не ходять у `data/`.
  invoicing: 4,
