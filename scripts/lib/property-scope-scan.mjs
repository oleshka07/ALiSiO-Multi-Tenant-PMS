...
const NAMED_IN_FILTER = [
  ...
  // `COALESCE(property_id, '') = ?` — третя нуль-безпечна форма, і додана вона
  // з тієї ж причини, що дві попередні (INC-038, 10.09.2026).
  //
  // Так звужують НУЛЬОВУ колонку там, де порожній рядок означає «на весь
  // рахунок»: `= ?` викидає рядки з NULL, а `IS NOT DISTINCT FROM ?` не годиться
  // там, де той самий вираз стоїть і в УНІКАЛЬНОМУ ІНДЕКСІ — саме так написані
  // `idx_invoice_series_row`, `idx_invoice_counters_row` і
  // `idx_channel_rate_rules_row`, бо жоден рушій не вважає NULL рівним NULL.
  // Запит мусить збігатися з індексом слово в слово, інакше він його не бере.
  //
  // Без цього рядка гейт називав ці читання «мовчазними» — тобто вимагав
  // переписати ПРАВИЛЬНЕ звуження на `= ?`, а це регресія: рядок рахунку зник
  // би з екрана. §3.2.1: хибно-червоний гейт лагодиться в гейті, а не в коді,
  // який він судить.
  new RegExp(String.raw`COALESCE\s*\(\s*\w*\.?property_id\s*,[^)]*\)\s*(?:=|<>|!=)\s*${PARAM}`, 'i'),
];
