...
const AXES = [
  ...
  // INC-036. Ціна послуги — це те, що поїде в рахунок гостя. З однаковими
  // цінами «нарахували свою» істинне й тоді, коли нарахували сусідову: у
  // рядку фоліо стоїть число, а не назва обʼєкта.
  {
  ...
  // INC-037 тут навмисно НЕМАЄ, і це не пропуск. Суми збору в
  // `registry-scope.check.ts` — не літерали, а вирази
  // (`fx.a.reservationIds.length * fx.a.cityTaxPerNight`), тож `distinct` лічив
  // би РІЗНІ ТЕКСТИ і був би зелений навіть тоді, коли числа однакові. Це
  // візерунок замість властивості (§3.2.1), тобто рядок, який не вміє
  // почервоніти. Вісь там тримає ПЕРШЕ ТВЕРДЖЕННЯ самої перевірки — воно
  // порівнює числа під час прогону і падає першим.
];
