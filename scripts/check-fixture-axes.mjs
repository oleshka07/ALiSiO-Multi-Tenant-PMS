...
const AXES = [
  ...
  // Блок 3 (К1): група кімнат. Сцена стверджує про три осі, і кожна з них
  // легко вироджується — саме так перша редакція сцени й була зеленою на
  // реалізації, яка впізнає кімнату позицією замість ключа.
  {
  ...
  // К4 (07.09): рідна форма групи додала дві осі, і обидві були виродженими
  // в першій редакції сцени. Порядок кімнат — доки кімната майстра стоїть
  // першою, «упізнав ключем» і «взяв позицію 0» дають те саме; фонд — доки
  // номерів кожного типу порівну, «зайнято по типах» збігається з «зайнято
  // всього, поділене навпіл».
  {
    file: 'src/modules/channels/data/inbound-bookings.check.ts',
