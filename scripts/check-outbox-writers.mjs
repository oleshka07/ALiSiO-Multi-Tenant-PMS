...
const ALLOWED = new Map([
  ...
  // Попередня бронь walk-in на терміналі (КІ8): `tentative` БЕЗ номера і БЕЗ
  // типу номера, тож вона не займає ні конкретної кімнати (`occupiedSpans`
  // дивиться `unit_id`), ні ємності типу (`unassignedByTypeDay` вимагає
  // `unit_type_id IS NOT NULL`). Наявність від неї не змінюється НІЯК —
  // і не мусить: кімнату продала чужа система, яка веде цей готель, і
  // порахувати її ще й у себе означало б відняти одну й ту саму кімнату
  // двічі. Щойно бронь дістає номер (`assignUnit` при заселенні), координата
