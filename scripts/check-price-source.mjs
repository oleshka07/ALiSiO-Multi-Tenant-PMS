...
function walk(dir) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Коментарі геть — ТУТ ТЕЖ.
    //
    // Стрипер у цьому файлі був, і шапка над ним пояснює на тридцять рядків,
    // чому без нього гейт рахує власну документацію (AGENTS §4, тричі за одну
    // сесію). Але кликало його лише ДРУГЕ правило: перше читало сирий текст,
    // тобто рівно ту помилку, від якої стрипер і писався.
    //
    // Спіймалось 12.09.2026 на реченні «прямого DELETE FROM price_calendar тут
    // немає навмисно» — коментарі, який ПОЯСНЮЄ, що заборонене не робиться.
    // Гейт показав на нього пальцем і зажадав виправити.
    //
    // Це хибно-червоний, і лагодиться він у гейті (AGENTS §3.2.1, випадок
    // сьомий): переписати коментар так, щоб гейт його не бачив, означало б
    // лишити гейт таким, що й далі не вміє того, чого не вміє, а в коді —
    // слід, який наступний читач прийме за норму.
